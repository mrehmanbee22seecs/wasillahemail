...
export type TemplateInput =
  | (BaseTemplate & { type: 'verification'; link: string; name?: string })
  | (BaseTemplate & { type: 'forgot-password'; link: string; name?: string })
  | (BaseTemplate & { type: 'welcome'; name: string; role?: string })
  | (BaseTemplate & {
      type: 'project-submission' | 'project-approval';
      name: string;
      projectName: string;
      submissionType: 'project' | 'event';
    })
  | (BaseTemplate & { type: 'reminder'; name: string; projectName: string; message: string })
  | (BaseTemplate & { type: 'volunteer-confirmation'; name: string })
  | (BaseTemplate & {
  ...
  | (BaseTemplate & { type: 'notification'; title: string; body: string; ctaLabel?: string; ctaUrl?: string });
...
const ROLE_MESSAGES: Record<string, { title: string; body: string; cta: string; ctaLink: string }> = {
  student: {
...
function wrapEmail(body: string, heading?: string) {
  return `
...
export function buildEmailTemplate(input: TemplateInput): TemplateOutput {
  switch (input.type) {
    ...
    default:
      throw new Error(`Unsupported template type: ${(input as BaseTemplate).type}`);
  }
}
